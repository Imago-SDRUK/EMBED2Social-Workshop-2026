...
::: {.notes}
We live in an era when we have an enormous amount of data. But that becomes our problem as well.
For your maybe simple routine task at your workplace you suddenly find out you have a few datasets in your institutional database
They are all different in resolution, accuracy, quality and etc. You spend a lot of time to sort it out which datasets are better for your purpose.
It's not your problem, not problem of data itself, it's just our reality that storages become storages of everything, or data swamps.
:::
...
::: {.notes}
  ...
  Google Earth Engine already provides ready simple visualisations of embeddings, but again it's the crucial point we have many bands to visualise.
  Here, you can exlore how your area of interest is illsutrated by embeddings and probably see some real-world objects, like built-up areas and forests.
  However, it's a sort of a very rough illustration.
:::
...
::: {.notes}
Currently, Google provides annual data from 2017 to 2024, but in the future seasonal or monthly products can be made available 
which will increase the range of applications massively. Currently, we have the 65th dimension which could vary from 2017 to 2024. 
And it's even a cool timeframe for ecologists and environmental geographers. For social scientists with this very dynamic research object, 
...
::: {.notes}
You can also explore the histogram of values distribution. We will work with 2024, but the same procedure for 2024.
Make sure layers are in EPSG:27700 Projected coordinate system for United Kingdom (UK).
:::
...
::: {layout-ncol=1}
![](assets/figures/lab1_exercise1_screen0-1.jpg){width=100%}
:::

::: {.notes}
Any difference between dimensions? They are, but seems to be quite illogical, can't be explained clearly.
:::
...
::: {layout-ncol=2}
![](assets/figures/lab1_exercise1_screen0-2.jpg){width=60%}
:::
...
::: {.notes}
As you can see, even one band doesn't follow any particular patterns like assigning negative values to all residential areas and positive 
values to forests and moors. It recognises the patterns between spatial features, relationships between them.
:::
...
::: {.notes}
Two tables with many attributes, so you need to find the same columns in two tables and compare them.
:::
...
::: {.notes}
You can join all columns from 2020, but I would recommend to pick up only required ones (A00_mean to A63_mean)
just not to mess up the output. Output file name could be custom as you wish.

What is much more important - define a prefix to joined columns from 2020, otherwise you can't separate 2024 from 2020 values.
:::
...
- calculate the difference between 2024 and 2020 for the `A00_mean` dimension: <br>
["A00_mean" - "2020_A00_mean"]{.orange} <br>
...
::: {.notes}
This stylisation is custom and you can apply another style to investigate the output layer, but this one is recommended.
:::
...
::: {.notes}
In this dimension (A00), the centre and outskirts change in a different way. East London is generally also under the negative dynamics
Inner city parks like Hyde Park or Regent Park are quite stable,
But some of the almost central densely built-up areas might have completely different direction of changes.

So, does this result confirm 'urban sprawl or 'gentrification'? Not really, it's only one band!
:::
...
::: {.notes}
This dimension looks completely different, isn't it? It illustrates many outskirt areas which are under intensive urban extension.
Many parks still stay stable.
One of the red stains on this map (red doesn't mean negative in ecological sense) is rainham marshes in the east.
I don't have enough knowledge about the London agglomeration, so if anyone knows what is happening there, it would be very interesting to hear.
:::
...
What regions behave similarly?
In terms of math its vector proximity operation.

Two locations are almost never exactly the same.
GPS coordinates differ a lot but their essense is very similar
Regions have fuzzy boundaries
Environmental and human activity patterns vary gradually
“Places that are close enough in meaning or behaviour”
:::
...
2. Go to <a href="https://imago-sdruk.github.io/EMBED2Social-Workshop-2026/assets/imago-similarity-explorer.html" target="_blank">
  https://imago-sdruk.github.io/EMBED2Social-Workshop-2026/assets/imago-similarity-explorer.html 
...
::: {.notes}
We can explore multiple dimensions, but we should be reasonable (2-3)
:::
